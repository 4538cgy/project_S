...
//    }
    ...
    fun sendMessageForPersonal(destinationUid : String?){
    ...
    fun sendMessageForOpen(){
        ...
        viewModelScope.launch(Dispatchers.IO) {
            ...
            if (chatRoomUid.value == null) {
                ...
                repository.createOpenChatRoom(chatRoomTitle,chatDTOs).collect {
                    if (it != null) println("채팅방 생성 성공") else println("채팅방 생성 실패")
                    println("채팅방 uid : " + it)
                    chatRoomUid.postValue(it)
                    //채팅방을 생성하고도 에딧텍스트에 값이 남아있다면 메세지 전달
